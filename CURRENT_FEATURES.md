...
**最終更新:** 2026-07-02（**フルモード調査（7/1 レポート以降の差分）**: ① **Layer 1 — CLI v2.1.198（7/1）**: **Claude in Chrome が GA** 告知 + **`Notification` フックの BG エージェント通知**（`agent_needs_input`/`agent_completed`）+ **新スキル `/dataviz`**（カラーパレット検証器同梱）+ **BG エージェントがコード完了時に自動 commit/push/ドラフト PR** + **Explore エージェントがメインセッションのモデル継承**（opus 上限）+ **サブエージェント/コンパクションが extended thinking 設定継承** + **Gateway に Claude Platform on AWS 追加** + **`/agents` ウィザード廃止** + ネットワーク耐性・BG タスク・エージェントチーム多数修正。新 CLI フラグ・新 Hook イベントなし。② **🆕🚨 Layer 2/3 — Claude Fable 5 / Mythos 5 アクセス復旧（7/1）**: 米商務省が 6/30 夜に輸出規制を解除、**Fable 5〔公開版〕は 7/1 グローバル復旧**（Claude Code 含む）、**Mythos 5 は米国機関限定で復旧**。**前回 7/1 レポートの『Fable 5 停止継続』を訂正**。③ **🆕 Layer 2 — Model Entitlements（Enterprise ベータ・7/1）**: 管理者がユーザーの利用可能モデル・effort レベルを制御。詳細は [2026-07-02 調査レポート](reports/2026-07-02_v2.1.198-chrome-ga-and-fable5-restored.md) 参照）
