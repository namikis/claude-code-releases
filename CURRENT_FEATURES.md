...
**最終更新:** 2026-05-05（**フルモード**: **v2.1.128（5/4 23:01 UTC リリース、約 45 項目の改善）** を反映。**v2.1.127 は欠番**（GitHub Releases / CHANGELOG.md ともになし、4/24 v2.1.124-125 同様の内部スキップと推察）。**Layer 1 主要変更**: ① **`--channels` がコンソール（API key）認証をサポート**（旧来 claude.ai login 限定、managed settings は `channelsEnabled: true` 必須）、② **`/color` 引数なしでランダム色選択**、③ **`/mcp` がツール数表示** + 0-tools サーバー警告マーク、④ **`--plugin-dir` が `.zip` プラグインアーカイブをサポート**、⑤ **`/model` ピッカーで Opus 4.7 重複削除**、現行 Opus は単に「Opus」表示、⑥ **サブプロセス（Bash / hooks / MCP / LSP）が `OTEL_*` 環境変数を継承しない**（OTEL 計装アプリの誤テレメトリ取得を防止）、⑦ **MCP `workspace` が予約サーバー名に**、⑧ **MCP 再接続時のツール一覧 flood を抑制**（server prefix サマリ）、⑨ **SDK ホストへ `localSettings` 提案**（Always allow が `.claude/settings.local.json` へ書き込まれる）、⑩ **`EnterWorktree` のブランチ作成元を `origin/<default-branch>` から local HEAD に修正**（未 push コミットドロップ回帰修正）、⑪ Auto mode classifier エラーヒント、⑫ **サブエージェント進捗サマリーのプロンプトキャッシュミス修正（cache_creation 約 3 倍 → 1/3 に削減）**、Bedrock global.* 回帰修正、vim Space 標準準拠、OSC 9;4 progress indicator ちらつき修正、1M context autocompact false-block 修正、並列シェル read-only コマンド失敗時の sibling キャンセル修正など 45 項目以上のバグ修正。**Layer 2 重大発表（5/4）**: **Anthropic × Blackstone × Hellman & Friedman × Goldman Sachs が $1.5B Enterprise AI サービス会社を共同設立**（Anthropic / Blackstone / H&F が各 $300M コミット、GIC / Apollo / General Atlantic / Leonard Green / Sequoia がコンソーシアム参加）。**Anthropic 自身が「コンサルティング業界に切り込む」**戦略的ピボット — エンジニアを中規模企業内に派遣し、Claude を中核業務へ embed、PE 傘下のヘルスケア / 製造業 / 金融 / リテール / 不動産が初期ターゲット。同日 OpenAI も類似 JV を発表、両社が AI 実装サービス領域へ垂直展開。**Layer 3**: TestingCatalog（5/1）が **Jupiter-v1-p（Anthropic 内部コードネーム惑星名）のレッドチームテスト**を報告 — 5/6 Code with Claude SF キーノートで発表期待。本日 5/5 は **The Briefing: Financial Services（11:00 AM EST、NYC livestream、C-suite 向け）**を予定通り開催）
...
- **確度**: 📢 公式アナウンス済み（Anthropic events ページに掲載）。**本日（5/5）開催**
...
- **文脈**: 2025-11 Anthropic Financial Services Plugins、2026-04-10 FRB Powell + 財務長官 Bessent + 米主要銀行 CEO の Mythos AI サイバー脅威協議、4/13 Wall Street Banks Mythos トライアル、4/29 $50B/$900B 調達ラウンド（Goldman Sachs / JPMorgan / Morgan Stanley が IPO 主幹事候補）の流れ。**5/4 Anthropic-Blackstone-H&F-Goldman Sachs $1.5B Enterprise AI JV 発表の翌日**、IPO ロードショー兼金融セクター戦略明示の場として注目度高い
- **最終確認日**: 2026-05-05
...
- **追加シグナル（5/5）**: TestingCatalog が 5/1 に **Anthropic Jupiter-v1-p（惑星名コードネーム）レッドチーム実施**を報告。2025-05 の Neptune コードネーム → Claude 4 ファミリー発表のパターンを踏襲する場合、**5/6 SF キーノートでの新モデル発表確度がさらに上昇**。Opus 4.7 は存在するが Sonnet 4.7 / Haiku 4.7 が欠落しているため、Sonnet ティアリフレッシュ説と Mythos foundation 上の新世代説が並立
- **最終確認日**: 2026-05-05
