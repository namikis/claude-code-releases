...
**最終更新:** 2026-07-05（**フルモード調査（7/4 レポート以降の差分）**: 新規 CLI **v2.1.201〔7/3 23:50〕** の 1 本のみ。前回 7/4 レポートが v2.1.199／v2.1.200 までを扱い **v2.1.201 が未捕捉だった**ため反映。**⚙️ v2.1.201 は単一エントリの極小リリース = Claude Sonnet 5 セッションがハーネスリマインダー（`<system-reminder>`）の配信に会話途中の system ロールを使わなくなった**（Sonnet 5 のプロンプト取り扱いに合わせた内部配線の最適化・ユーザー操作/設定/UI 変更なし）。**新機能・新 CLI フラグ・新スラッシュコマンド・新 Hook・バグ修正エントリはいずれもなし**。**Layer 2/3 は 7/4〜7/5 に Claude Code 固有の新機能・新モデル・新ティアの公式発表なし**（公式ニュースルーム直近は 7/2『Fable 5 cyber safeguards / jailbreak framework』、7/3〜7/5 新規投稿なし。検索上位の「Claude's new constitution」は 2026-01-22 の既出で期間外）。詳細は [2026-07-05 調査レポート](reports/2026-07-05_v2.1.201-sonnet5-harness-reminder-system-role.md) 参照。以下は前回まとめ→ **フルモード調査（7/3 レポート以降の差分）**: 新規 CLI **v2.1.199〔7/2〕・v2.1.200〔7/3〕** の 2 本。**⚙️ v2.1.200 でデフォルト権限モードを「default」→「Manual」に改称**（CLI/`--help`/VS Code/JetBrains 統一、`--permission-mode manual`・`"defaultMode": "manual"` を `default` と併せ受理）。**⚙️ `AskUserQuestion` ダイアログが既定で自動継続しなくなった**（アイドルタイムアウトは `/config` オプトイン）。**🔁 v2.1.199 でリトライ堅牢化**: 一時的 429〔利用上限無関係〕をサブスクライバー向けにバックオフ自動リトライ、`CLAUDE_CODE_RETRY_WATCHDOG` の非容量系デフォルトリトライを 300 に、`CLAUDE_CODE_MAX_RETRIES` の上限 15 を撤廃。**🧩 スタック済みスラッシュスキル**（`/skill-a /skill-b …`）で先頭最大 5 スキルをロード。SSL 証明書エラー即時失敗、サブエージェント/ストリーミングのエラー伝播修正、Linux デーモン ~50 秒自己 kill 修正、BG デーモン安定化多数、tmux 3.4+ ちらつき修正、スクリーンリーダー改善。新スラッシュコマンド・新 Hook なし。Layer 2/3 は 7/3〜7/4 に Claude Code 固有の新機能発表なし。詳細は [2026-07-04 調査レポート](reports/2026-07-04_v2.1.199-200-manual-permission-default-and-retry-watchdog.md) 参照。以下は前回まとめ→ **ニュースモード調査（7/2 レポート以降の差分）**: 新規 CLI リリースなし（Layer 1 最新 v2.1.198／7/1、Layer 1 スキップ）。**🆕 Layer 2 — Claude apps gateway（6/29 公式ブログ・✅ 提供開始）**: Claude Code をエンタープライズ展開する**自己ホスト型コントロールプレーン**〔企業 SSO・中央集権ポリシー・RBAC・ユーザー単位コスト按分・Claude API/Bedrock/Google Cloud への上流ルーティング＋フェイルオーバー・支出上限・単一ステートレスコンテナ＋PostgreSQL・既存 `claude` バイナリ同梱・プロトコル公開〕。v2.1.198 の『Gateway に Claude Platform on AWS 追加』の親製品にあたる本体を新規追加。**🆕 Layer 2/3 — Claude Science（6/30 公式・✅ 提供開始）**: 科学者・製薬向け AI ワークベンチ〔60+ キュレーション済みスキル/コネクタ・社内創薬プログラム併発表・AI for Science 支援は 7/15 まで〕＝Claude Code の隣接プロダクト。**🆕 Layer 3 — Claude Code から競合検知隠しコード（ステガノグラフィ）を削除（7/1 The Register）**: 2026-03 導入の不可視 Unicode マーカー＋エンコード済みドメインリスト〔中国 AI ラボ・リセラー・ゲートウェイ検知〕を 7/1 リリースで撤去。詳細は [2026-07-03 調査レポート](reports/2026-07-03_claude-apps-gateway-claude-science-and-covert-code-removal.md) 参照）
...
- **🆕 Trusted Devices for Remote Control 🧪 ベータ（2026-06-25）**: **Team / Enterprise** の管理者が、メンバーが**ローカル Claude Code セッションをリモート（claude.ai / Claude モバイルアプリ / Claude Desktop）から閲覧・操作する前にデバイス検証を必須化**できる組織横断設定。アクセスを「サインイン済みアカウント」だけでなく「既知のデバイス＋最近の認証」に紐付ける。各ブラウザ/スマホ/デスクトップアプリが個別に資格情報を登録（**フルサインイン直後にのみ登録提示**、バックグラウンドでの暗黙登録なし）。**デフォルトはオフ**（管理者が明示有効化するまで無効）。情報源: [Release notes — Claude Help Center（6/25）](https://support.claude.com/en/articles/12138966-release-notes) / [Remote Control — Claude Code Docs](https://code.claude.com/docs/en/remote-control)
...
| `Notification` | 通知イベント。**v2.1.198 で `claude agents` のバックグラウンドエージェントが入力要求時 `agent_needs_input` / 完了時 `agent_completed` を発火** |
...
### Claude in Chrome / Chrome 拡張 ✅（2026-07-01 GA）
- **リリース状態**: ✅ GA（v2.1.198 changelog「Claude in Chrome is now generally available」）。対象は Pro / Max / Team / Enterprise（有料プラン）。2025-08 リサーチプレビュー → 2025-11〜12 有料プランベータ → **2026-07-01 GA**
- ブラウザサイドパネルで Claude が Web ページを読む・クリック・ナビゲート。**Claude Code 連携**（ターミナルでビルド → ブラウザで検証、コンソールエラー・DOM 状態を読んでデバッグ）。プロンプトインジェクション防御を強化（攻撃成功率 23.6%→11.2%、追加緩和でブラウザ特化型攻撃 0%）
- `--chrome` フラグでブラウザ自動化・Web テスト・ライブデバッグが可能。Team/Enterprise は管理者が組織横断で有効/無効化・サイト allowlist/blocklist 設定可
- **情報源**: [CHANGELOG](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md) / [Get started with Claude in Chrome](https://support.claude.com/en/articles/12012173-get-started-with-claude-in-chrome)
...
| `sandbox.credentials`（設定） | サンドボックス実行されたコマンドが**資格情報ファイル・シークレット環境変数を読み取れないようにブロック**。サンドボックスからのデータ流出防止を強化（v2.1.187） |
| 組織モデル制限の UI 露出 | 組織設定（`availableModels` / `enforceAvailableModels`）のモデル制限を **`/model` ピッカー・`--model`・`/model`・`ANTHROPIC_MODEL` に反映**。制限モデル選択時に「restricted by your organization's settings」を表示（v2.1.187） |
| `CLAUDE_CODE_MCP_TOOL_IDLE_TIMEOUT`（環境変数） | リモート MCP ツール呼び出しが**無応答 5 分でエラーアボート**（無限ブロック防止）。タイムアウト値をオーバーライド可能（v2.1.187） |
| `CLAUDE_CODE_MAX_RETRIES` / `CLAUDE_CODE_RETRY_WATCHDOG` | リトライ回数の上限を **15** に変更、無人セッションでは代わりに `CLAUDE_CODE_RETRY_WATCHDOG` を使用（v2.1.186）。**v2.1.199 で `CLAUDE_CODE_RETRY_WATCHDOG` は非容量系の一時エラーのデフォルトリトライ回数を 300 に引き上げ、`CLAUDE_CODE_MAX_RETRIES` の上限 15 を撤廃**。加えて一時的な 429〔利用上限とは無関係〕がサブスクライバー向けにバックオフ付きで自動リトライされるようになった（v2.1.199） |
| `autoMode.classifyAllShell`（設定） | Auto mode で**全 Bash/PowerShell コマンド**を auto-mode 分類器に通す（デフォルトは任意コード実行パターンのみ評価）。より保守的な自動実行ポリシー向け（v2.1.193） |
| `OTEL_LOG_ASSISTANT_RESPONSES`（環境変数） | 新 OTEL ログイベント `claude_code.assistant_response`（アシスタント応答テキスト）の送出を制御。**未設定時は `OTEL_LOG_USER_PROMPTS` に追従**するため、プロンプトをログ済みの環境はアップグレードで応答内容も送出開始。プロンプトのみに留めるには `=0`（v2.1.193） |
| `CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP`（環境変数） | `=1` で、アイドルなバックグラウンドシェルコマンドのメモリ圧迫時自動リープ（回収）を無効化（v2.1.193） |
| `CLAUDE_CODE_DISABLE_MOUSE_CLICKS`（環境変数） | フルスクリーンモードでのマウスクリック/ドラッグ/ホバーを無効化（ホイールスクロールは維持）。v2.1.187 で入ったフルスクリーンのマウスクリック対応をオプトアウトし、ターミナルのテキスト選択/コピペとの干渉を回避（v2.1.195） |
| `--permission-mode manual` / `"defaultMode": "manual"`（設定） | **デフォルト権限モードの名称が「default」→「Manual」に統一**（CLI・`--help`・VS Code・JetBrains）。`manual` が既存の `default` と併せて受理される（後方互換維持、v2.1.200） |
| `AskUserQuestion` 自動継続の既定オフ | `AskUserQuestion` ダイアログが**既定で自動継続しなくなった**（放置で既定選択へ進まない）。アイドルタイムアウトを使うには `/config` でオプトイン（v2.1.200） |
...
| `/rewind` | チェックポイントへ巻き戻し（`/undo` はエイリアス、v2.1.108）。**v2.1.191 で `/clear` 実行前の地点からの会話再開に対応** |
...
| `/dataviz` | **チャート・ダッシュボード設計ガイダンススキル**。実行可能なカラーパレット検証器を同梱（v2.1.198） |
...
| 項目 | 詳細 |
|------|------|
| **デフォルトモデル** | **Claude Sonnet 5（2026-06-30 GA、モデル ID `claude-sonnet-5`）— Claude Code は v2.1.197（6/30）でデフォルトを Opus 4.8 → Sonnet 5 に切替**（1M トークンコンテキストがデフォルト・最大出力 128k・adaptive thinking デフォルト ON・要 v2.1.197+）。claude.ai は Free/Pro のデフォルト。**旧デフォルトの Claude Opus 4.8（2026-05-28 GA、`claude-opus-4-8`、デフォルト effort「high」）は `/model` で選択継続**（v2.1.154〔5/28〕〜v2.1.196 は Claude Code デフォルト＝高難度タスク向けエスカレーション、Max / Team Premium / Enterprise pay-as-you-go / Anthropic API でデフォルト）。（さらに旧: 2026-04-16〜 Opus 4.7、4-23 より API デフォルトも Opus 4.7） |
...
| **Claude Codeデフォルト努力レベル** | **Sonnet 5（v2.1.197〜デフォルト）は adaptive thinking がデフォルト ON**（`thinking: {type:"disabled"}` でオフ、effort パラメータで制御）。**`high`（Opus 4.8 使用時、v2.1.154〜）**。`ultracode`（effort menu 経由）で effort を `xhigh` にしつつワークフロー利用を Claude に自動判断させる。（旧: Opus 4.7 使用時は `xhigh`） |
...
## Claude apps gateway（自己ホスト型コントロールプレーン）✅
- **リリース状態**: ✅ 提供開始（2026-06-29 公式ブログ、"available now"）
- **対象**: Enterprise / Team（自己ホスト運用）
- **概要**: Claude Code をエンタープライズ展開するための**自己ホスト型コントロールプレーン**。**企業 SSO（OIDC/IdP）ログイン・中央集権的なマネージド設定の配布/適用・ロールベースアクセス・ユーザー単位のコスト按分**を提供。上流クレデンシャルを保持し推論を **Claude API / Amazon Bedrock / Google Cloud** へルーティング（プロバイダ間フェイルオーバー付き、v2.1.198 で `anthropicAws`＝Claude Platform on AWS を上流に追加）。日次/週次/月次の**支出上限**（組織/グループ/ユーザー単位）、使用量レポート。Claude API を明示設定しない限り推論トラフィック・使用量を Anthropic へ送信しない。
- **アーキテクチャ**: Linux 上の**単一ステートレスコンテナ**＋PostgreSQL。**既存の `claude` バイナリに同梱**（`gateway.yaml` に OIDC issuer・上流クレデンシャルを設定、IdP に OIDC アプリを 1 つ登録）。ゲートウェイのプロトコルを公開しサードパーティ実装を許容。Microsoft Foundry も docs 上で対応記載。
- **初出バージョン/日付**: 2026-06-29（公式ブログ）
- **情報源**: [Introducing the Claude apps gateway](https://claude.com/blog/introducing-the-claude-apps-gateway) / [Claude apps gateway（Docs）](https://code.claude.com/docs/en/claude-apps-gateway)

## Claude Sonnet 5 ✅
- **リリース状態**: ✅ GA（2026-06-30）。claude.ai（Free/Pro のデフォルト）/ Claude API（`claude-sonnet-5`、全顧客）/ Claude Code（v2.1.197〜デフォルト）/ Cursor / VS Code / GitHub Copilot / Amazon Bedrock ＋ Claude Platform on AWS（legacy Bedrock の `InvokeModel`/`Converse` は非対応）/ Google Cloud（Vertex AI）/ Microsoft Foundry（preview）
- **位置づけ**: Sonnet 系次世代モデルで **Claude Sonnet 4.6 のドロップイン後継**。「速度と知能の最良の組み合わせ」を掲げ、**エージェント能力を業界標準の価格帯へ引き下げる**戦略モデル。Opus 4.8 に迫る品質を大幅に低いコストで提供
- **Claude Code デフォルト化**: **v2.1.197（6/30）で Claude Code のデフォルトを Opus 4.8 → Sonnet 5 に切替**（5/28 v2.1.154 以来の Opus デフォルトを覆すコスト効率重視の転換、要 v2.1.197+、Opus 4.8 は `/model` で選択継続）
- **コンテキスト / 出力**: **1M トークンコンテキストがデフォルト（かつ最大。小容量バリアントなし）**、**最大出力 128k トークン**
- **価格**: **導入価格 $2/$10 per MTok（入力/出力、2026-08-31 まで）** → 以降 **標準 $3/$15 per MTok**（Sonnet 4.6 から据え置き）。⚠️ 新トークナイザで同一テキストのトークン数が増えるため同等リクエストの実コストは 4.6 と異なり得る
- **ベンチマーク**: **SWE-bench Pro（エージェント型コーディング）63.2%**（Sonnet 4.6: 58.1% / Opus 4.8: 69.2% / GPT-5.5: 58.6% / Gemini 3.5 Flash: 55.1%、Sonnet 首位）、Terminal-Bench 2.1 **80.4%**（GPT-5.5 83.4% がわずかに上回る）、**GDPval-AA v2（ナレッジワーク）1,618（Opus 4.8 の 1,615 をわずかに超え）**
- **⚠️ 挙動変更（Sonnet 4.6 からの移行時）**: ① **adaptive thinking がデフォルト ON**（`thinking: {type:"disabled"}` でオフ、`max_tokens` は thinking＋応答の合計上限のため要見直し）② **手動 extended thinking 廃止**（`thinking: {type:"enabled", budget_tokens}` は 400 エラー、Opus 4.8/4.7 と同様、effort パラメータへ移行）③ **sampling パラメータ非対応**（`temperature`/`top_p`/`top_k` 非デフォルト値は 400、Opus 4.7 で先行導入）④ **新トークナイザ**（同一テキストが約 30% 多いトークン、Opus 4.7 導入のもの。API 形状不変・コード変更不要だが `usage`/token counting/1M 実容量/`max_tokens` 予算/単価が影響、旧モデル基準の計測値は再計測要）。assistant メッセージ prefill は引き続き非対応
- **安全性**: 欺瞞・ハルシネーション率が前世代より低下、悪意リクエスト拒否とプロンプトインジェクション耐性が向上。**Sonnet ティア初のリアルタイムサイバーセキュリティセーフガード**（禁止・高リスクなサイバー話題は拒否＝HTTP 200 + `stop_reason: "refusal"`）
- **Priority Tier**: Sonnet 5 では非対応（4.6 では対応）。ZDR 契約組織は zero data retention 対応
...
## Claude Fable 5 / Claude Mythos 5 ✅（2026-07-01 アクセス復旧）
- **⛔ 重要（2026-06-12〜）**: **米国政府の輸出管理指令により、両モデルとも全ユーザー向けにアクセス停止中**。指令（6/12 17:21 ET 受領）は「米国内外を問わずあらゆる外国籍者（外国籍 Anthropic 従業員含む）によるアクセスの停止」を要求し、Anthropic は外国籍ユーザーを米国ユーザーからリアルタイムに選別できないため**全ユーザーで両モデルを無効化**。発端は 6/10 に X 上で公開された Fable 5 の jailbreak（サイバー攻撃・爆発物・化学合成経路の情報抽出を主張）。Anthropic は「狭い jailbreak が商用モデルのリコール理由になるべきではない」と不同意を表明し「できる限り早く復旧する」とコメント。**Opus 4.8 を含む他の全モデルは影響なし**。**2026-06-16 時点でも公式の復旧日は未定で、両モデルとも全ユーザー向けに利用不可のまま継続**（AWS Bedrock 上の Fable 5 / Mythos 5 も revoke 済み）。 **【2026-06-26 更新 — Mythos 5 の輸出規制部分解除】** 米商務長官 **Howard Lutnick** が 6/26（金）付の Anthropic 宛て書簡で、**Claude Mythos 5 を 100 を超える「信頼できる」米国機関**（Project Glasswing 参加の著名テック企業約 100 社＋連邦政府機関、クリティカルインフラの運用・防衛主体）へ展開する許可を通知（6/12 指令以来 2 週間の交渉を経て）。**書簡は Fable 5〔公開版〕に言及せず、Fable 5 は ⛔ 停止継続**（解放交渉は継続も時期不明）。**これは輸出管理上の政策決定（許可付与）であり、各機関への実展開・製品再有効化の完了を直ちには意味しない**（一般ユーザー・国際ユーザー・公開版 Fable 5 は対象外）。報道ベース（[CNBC 6/26](https://www.cnbc.com/2026/06/26/us-government-anthropic-claude-mythos5-ai.html) / [9to5Mac 6/26](https://9to5mac.com/2026/06/26/anthropic-cleared-to-release-claude-mythos-5-to-over-100-us-institutions/) / [The Hill](https://thehill.com/policy/technology/5943549-anthropic-mythos-5-access/) / [BeInCrypto](https://beincrypto.com/anthropic-mythos-5-export-block-lifted/) が一致）、Anthropic 公式ブログでの正式アナウンス・実展開タイムラインは 6/28 時点未確認。Claude Code では `/model fable` 等は当面エラー/フォールバックになる見込み（デフォルトは元々 Opus 4.8 のため通常ワークフローへの影響は限定的）。[Anthropic 公式声明](https://www.anthropic.com/news/fable-mythos-access) **【2026-06-18 更新】** Anthropic がソウルオフィス開設記者会見で、国際担当 MD **Chris Ciauri** が「**数日以内（in the coming days）に両モデルは再び利用可能になると非常に確信している**」と公式に発言（復旧日・対象範囲＝US 限定か全面かは未確定、Project Glasswing についてはコメントを回避）。韓国メディアは、Mythos アクセス予定先に中国と潜在的関係を疑われる韓国通信会社（**SK Telecom**、2023 年に Anthropic へ $100M 出資・Glasswing 約 150 組織の 1 社）が含まれたことが 6/12 指令の引き金と報道、同社の Mythos アクセスは米政府要請で取り消されたとされる。 **【2026-06-20 更新】** 6/20 時点も両モデルとも全ユーザー向けにアクセス停止のまま。Anthropic の上級技術スタッフがワシントンの**商務省（Department of Commerce）当局者と協議中**で、Ciauri の「数日以内に復旧と確信」発言は維持。予測市場（Kalshi）は **7/1 までの復旧を約 57%、7/10 までを約 67%、7/17 までを約 75%** と織り込み、復旧日・対象範囲は依然未確定（[Kalshi News](https://news.kalshi.com/p/fable-5-odds-anthropic-access-restored-july-57-percent) / [Octagon](https://octagonai.co/markets/politics/when-will-anthropic-restore-fable-5-access-for-us-customers/)）。
- **リリース状態（現況・2026-07-01〜）**: **🆕 Claude Fable 5〔公開版〕= ✅ グローバル復旧**（米商務省が 6/30 夜に輸出規制を解除、7/1 に Claude Platform / Claude.ai / Claude Code / Cowork でグローバル復旧。7/7 まで Pro/Max/Team/一部 Enterprise は週次上限の最大 50% までアクセス→以降 usage credits。新安全分類器が問題の jailbreak を 99% 超でブロック〔商務省 CAISI 独立テスト〕。[CoinDesk 7/1](https://www.coindesk.com/tech/2026/07/01/anthropic-restores-ai-models-fable-mythos-after-the-u-s-lifts-export-controls) / [9to5Mac 7/1](https://9to5mac.com/2026/07/01/claude-fable-5-cleared-to-return-as-us-lifts-anthropics-export-control-restriction/) / [CIO 7/1](https://www.cio.com/article/4191550/us-reverses-export-restrictions-on-anthropics-fable-5-mythos-5-ai-models.html)） / **Claude Mythos 5 = 🔬 承認済み米国機関限定で復旧**（6/26 承認、国内外パートナーへの拡大は継続調整中） **【本リポジトリ訂正】前回 7/1 レポートは『Fable 5 は 7/1 も ⛔ 停止継続』と記録していたが、7/1 グローバル復旧が確定したため更新**
...
### Claude Science（科学者向け AI ワークベンチ）✅ — 隣接プロダクト
- **確度**: ✅ 提供開始（2026-06-30 公式 "Claude Science, an AI workbench for scientists"）。**Claude Code の CLI 機能ではない隣接プロダクト**のためここに記録（Claude Code 連携が明確化した時点で上部へ移動を検討）。
- **概要**: Anthropic の LLM を科学ラボ・製薬研究向けに最適化したアプリ。研究データセット分析・論文レビュー・生物学データ解釈・計算ワークフロー管理・仮説生成を支援。**60+ のキュレーション済みスキル/コネクタ**（ゲノミクス/single-cell/プロテオミクス/構造生物学/cheminformatics）を同梱。Claude Code / Cowork と同じ Skills/Connectors 基盤を科学ドメインに展開（Amodei は「Claude Code 級のインパクト」と発言）。社内創薬プログラムも併発表。AI for Science 支援＝最大 50 プロジェクト・各最大 $30,000 クレジット、応募は 7/15 まで。
- **情報源**: [Claude Science（公式）](https://www.anthropic.com/news/claude-science-ai-workbench) / [STAT News（6/30）](https://www.statnews.com/2026/06/30/anthropic-release-claude-science-ceo-dario-amodei/) / [CNBC（6/30）](https://www.cnbc.com/2026/06/30/anthropic-launches-ai-drug-discovery-program-claude-science.html)
- **最終確認日**: 2026-07-03

### Claude Code の競合検知隠しコード（ステガノグラフィ）を削除 🔍
- **確度**: ✅ 撤去（7/1 の Claude Code リリース〔v2.1.198〕で削除、PR マージ済み。The Register 報道ベース、公式 changelog では明示されていない挙動変更）。
- **経緯**: Anthropic は **2026 年 3 月**に Claude Code のシステムコンテキストへ **ステガノグラフィ（不可視 Unicode マーカー＋エンコード済みドメインリスト）**を仕込み、`ANTHROPIC_BASE_URL`・システムタイムゾーンをチェックしホスト名を「既知の中国 AI ラボ・他 AI 企業・アカウントリセラー・ゲートウェイドメイン」のリストと突合してモデル蒸留/不正リセラーを検知していた（Thariq Shihipar）。「より強力な緩和策を実装済みで以前から撤去予定だった」として **7/1 リリースで撤去**の見込み。6/24 の Alibaba 蒸留告発と同じ能力保護の文脈で、エンタープライズ監査・プライバシー観点の論点になり得る。
...
- **確度**: 🧪 ベータ — 2026-06-23 Anthropic 公式ニュース「Introducing Claude Tag」で発表。**Claude Enterprise / Team 向けベータ**。
- **対象プラットフォーム**: Slack（動作モデル Opus 4.8）
- **概要**: Slack のチャンネルで `@Claude` をタグ付けしてタスクを委任できる **チーム共有型 AI ティームメイト**。タスクを段階分解して非同期実行し、保有ツールで作業、完了時に Slack スレッドへ成果を投稿。チャンネル内では 1 つの Claude を全員が共有（誰が何を依頼・進行中かが可視化、続きを引き継ぎ可）、チームの作業文脈を時間をかけて記憶（メモリ）、ambient モードで能動的に情報提示。**既存「Claude in Slack」アプリを置換**し、管理者は 30 日以内にオプトインで移行。Anthropic 自社では製品チームのコードの 65% を社内版 Claude Tag で生成と説明。Claude Code の CLI 機能ではないが、ツール・データ・コードベースに接続するマルチプレイヤー型エージェント運用を Slack 面に拡張する隣接プロダクト（公式確認が取れ Claude Code 連携が明確化した時点で本ドキュメント上部へ移動を検討）。
...
### Fable 5 のサブスク恒久同梱（週次 usage 上限）シグナル ❓
- **確度**: ❓ 噂・未確認（CLI 文字列のデータマイン）— ユーザーが **v2.1.190（6/24）の文字列差分**から、Fable 5 をサブスクリプションに **恒久同梱し週次 usage 上限を課す** 方向の準備を検出。新文字列 "You've used your Fable 5 usage for this week" が追加され、旧来の "purchased separately from your plan"（プランとは別購入）表現が削除された。
- **背景**: 6/22 で「サブスク同梱無料」が終了し 6/23 に usage credits 建てへ移行した直後の方針転換シグナル（プレミアムアドオン → サブスク標準同梱＋週次上限）と読める。ただし **課金パッケージのシグナルと、6/12 米政府輸出管理指令によるアクセス停止は別レイヤーの問題**であり、本シグナルは復旧日・復旧範囲を確定するものではない。Fable 5 / Mythos 5 自体は 6/25 時点も ⛔ アクセス停止継続（公式復旧アナウンスなし）。
...
- **⚠️ ステータス更新（2026-06-26）**: 米商務長官 Lutnick の 6/26（金）書簡により **Mythos 5 が 100+ 米国機関（Glasswing 約 100 社＋連邦機関、クリティカルインフラ）への展開を許可**（輸出規制の部分解除）。ただし **Fable 5〔公開版〕は書簡で言及されず停止継続**。これは政策上の許可付与であり実展開完了は別。詳細は本ドキュメント上部「Claude Fable 5 / Claude Mythos 5」セクション参照。情報源: [CNBC 6/26](https://www.cnbc.com/2026/06/26/us-government-anthropic-claude-mythos5-ai.html) / [9to5Mac 6/26](https://9to5mac.com/2026/06/26/anthropic-cleared-to-release-claude-mythos-5-to-over-100-us-institutions/)
...
| **サブスク課金構造変更（programmatic 利用の分離）** | **❌ 2026-06-15 一時停止＝施行されず（撤回・延期）** | **⚠️ 訂正**: 5/13-14 発表・6/15 発効予定だった本変更（Agent SDK / `claude -p` / Claude Code GitHub Actions / サードパーティ Agent アプリ をサブスク flat-rate プールから分離 → ユーザー単位ドル建て monthly credit メーター〔Pro 約 $20・Max 5x 約 $100・Max 20x 約 $200/月、ロールオーバーなし〕）は、**発効当日の 2026-06-15 に Anthropic が Help Center で「もはや実施しない（no longer happening）」と公表し一時停止**。これらの利用面は引き続き **Pro/Max/Team/Enterprise の通常サブスク上限から消費され変更なし**（クレジット付与もなし）。Anthropic は計画を練り直し中で、将来の変更時は事前告知すると表明。**前回まで本表に「6/15 施行済み」と記録していたが、実際には施行されず一時停止のため訂正** |
...
- 2026-07-05: **フルモード調査**（7/4 レポート以降の差分）。**Layer 1 — 新規 CLI は v2.1.201〔7/3 23:50〕の 1 本のみ**（前回 7/4 レポートは v2.1.199／v2.1.200 までを扱い、**v2.1.201 が未捕捉だった**ため反映、Layer 1 最新は v2.1.201）。**⚙️ v2.1.201 は単一エントリの極小リリース** = **Claude Sonnet 5 セッションが、ハーネスリマインダー（`<system-reminder>` 形式の内部注意喚起）の配信に会話途中（mid-conversation）の system ロールを使わなくなった**。6/30 Sonnet 5 GA 由来のプロンプト取り扱い（adaptive thinking デフォルト ON・新トークナイザ等）に合わせてリマインダー注入経路を最適化したもので、会話履歴中への system ロール混入によるモデル解釈ノイズ低減が狙いと見られる。**ユーザー操作・設定・UI 変更なし**、可視の挙動変化はない。**新機能・新 CLI フラグ・新スラッシュコマンド・新 Hook イベント・バグ修正エントリはいずれもなし**（BG デーモン安定化・tmux 3.4+ ちらつき修正・スクリーンリーダー改善等の大量修正は v2.1.200 に含まれ 7/4 レポートで反映済み）。**Layer 2/3 — 7/4〜7/5 に Claude Code 固有の新機能・新モデル・新ティアの公式発表なし**（Anthropic 公式ニュースルーム直近は 7/2『More details on Fable 5's cyber safeguards and our jailbreak framework』、7/3〜7/5 新規投稿なし。検索上位の「Claude's new constitution」は 2026-01-22 公開の既出記事で本期間とは無関係につき除外）。**ステータス更新**: v2.1.201 の Sonnet 5 ハーネスリマインダー配線変更 新規 → ✅ GA、v2.1.201 → ✅ GA。CLI フラグ・スラッシュコマンド・Hook イベントの追加はなし。**CURRENT_FEATURES.md**: ① ヘッダー書換のみ（新規テーブル行・新規セクションの追加該当なし＝内部挙動変更のため）（[調査レポート](reports/2026-07-05_v2.1.201-sonnet5-harness-reminder-system-role.md)）
- 2026-07-04: **フルモード調査**（7/3 レポート以降の差分）。**Layer 1 — CLI 2 本進行 = v2.1.199〔7/2〕・v2.1.200〔7/3〕**（Layer 1 最新 v2.1.200）。**⚙️ v2.1.200 — デフォルト権限モードを「default」→「Manual」に改称**（CLI・`--help`・VS Code・JetBrains で統一、`--permission-mode manual`・設定 `"defaultMode": "manual"` を `default` と併せ受理＝後方互換維持の用語統一）+ **⚙️ `AskUserQuestion` ダイアログが既定で自動継続しなくなった**（放置で既定選択へ進まず、アイドルタイムアウトは `/config` オプトイン）+ BG セッション/デーモン安定化多数（スリープ復帰後のサイレント中断・PID 再利用された古い `daemon.lock` で起動不能・古いビルドのデーモン乗っ取り〔埋め込みビルドタイムスタンプで判定〕・ロースター破損）・レート制限で出力前に打ち切られたサブエージェントの空結果・`.claude.json` の非配列 `disabledMcpServers`/`enabledMcpServers` 起動クラッシュ・`claude agents --plugin-dir` 後置時の非表示・git worktree からの project-scoped プラグイン誤ロード修正・**tmux 3.4+ ちらつき修正（synchronized output）**・スクリーンリーダー出力改善・音声「Voice connection failed」誤表示修正。**🔁 v2.1.199 — リトライ堅牢化**: 一時的 429〔利用上限無関係〕をサブスクライバー向けにバックオフ付き自動リトライ + `CLAUDE_CODE_RETRY_WATCHDOG` の非容量系デフォルトリトライを **300** に引き上げ・`CLAUDE_CODE_MAX_RETRIES` の上限 **15 を撤廃** + SSL 証明書エラー〔TLS 検査プロキシ・`NODE_EXTRA_CA_CERTS` 欠落・期限切れ〕を即時失敗＋修正ヒント提示。**🧩 スタック済みスラッシュスキル**（`/skill-a /skill-b do XYZ`）で先頭最大 5 スキルをロード（従来は先頭 1 つ）。**サブエージェント/ストリーミングのエラー伝播修正**: ミッドストリーム overloaded/サーバーエラーで部分出力を保持・打ち切られたサブエージェントが部分成果を親へ返す・API エラー（usage limit 等）を成功結果ではなく親へ正しく報告。Linux BG デーモンの ~50 秒自己 kill 修正・macOS SSH コールドスタート回帰（v2.1.196 由来）修正・プランモードで状態変更ブラウザツールを正しくプロンプト（読み取り専用 `browser_batch` は自動許可）・`claude agents` の PR リンクを `#N` 表示化。**新スラッシュコマンド・新 Hook イベントの追加なし**。**Layer 2/3 — 7/3〜7/4 に Claude Code 固有の新機能・新モデル・新ティアの公式発表なし**（公式ニュースルーム直近は 7/2『More details on Fable 5's cyber safeguards and our jailbreak framework』＝Fable 5 安全フレームワーク解説で CLI とは別軸、Fable 5/Mythos 5 復旧は前回 7/2 既出）。**ステータス更新**: デフォルト権限モード「Manual」改称 / `AskUserQuestion` 自動継続オフ / 一時 429 自動リトライ / `CLAUDE_CODE_RETRY_WATCHDOG` 300・`MAX_RETRIES` 上限撤廃 / スタック済みスラッシュスキル最大 5 ロード 新規 → ✅ GA、v2.1.199・v2.1.200 → ✅ GA。**CURRENT_FEATURES.md**: ① ヘッダー書換、② CLI フラグ表の `CLAUDE_CODE_MAX_RETRIES`/`CLAUDE_CODE_RETRY_WATCHDOG` 行を v2.1.199 の挙動変更で更新、③ 同表に `--permission-mode manual`/`defaultMode: manual` 改称・`AskUserQuestion` 自動継続既定オフの 2 行を追記（[調査レポート](reports/2026-07-04_v2.1.199-200-manual-permission-default-and-retry-watchdog.md)）
- 2026-07-03: **ニュースモード調査**（7/2 レポート以降の差分）。新規 CLI リリースなし（Layer 1 最新 v2.1.198／7/1、Layer 1 スキップ）。**🆕 Layer 2 — Claude apps gateway（6/29 公式ブログ・✅ 提供開始）**: Claude Code をエンタープライズ展開するための**自己ホスト型コントロールプレーン**。企業 SSO（OIDC/IdP）ログイン・中央集権的なマネージド設定配布/適用・ロールベースアクセス・ユーザー単位のコスト按分を提供、上流クレデンシャルを保持し Claude API/Amazon Bedrock/Google Cloud へルーティング（プロバイダ間フェイルオーバー付き）、日次/週次/月次の支出上限、単一ステートレスコンテナ＋PostgreSQL、**既存の `claude` バイナリに同梱**、プロトコル公開。前回 v2.1.198 changelog の『Gateway に Claude Platform on AWS（anthropicAws）追加』の親製品本体が未記載だったため新規追加。**🆕 Layer 2/3 — Claude Science（6/30 公式・✅ 提供開始）**: 科学研究者・製薬向けの新フラッグシップ「AI ワークベンチ」。60+ のキュレーション済みスキル/コネクタ（ゲノミクス/single-cell/プロテオミクス/構造生物学/cheminformatics）を同梱、社内創薬プログラム（neglected 疾患）併発表、AI for Science 支援＝最大 50 プロジェクト・各最大 $30,000 クレジット（応募 7/15 まで）。**Claude Code の CLI 機能ではない隣接プロダクト**だが同じ Skills/Connectors 基盤を科学ドメインに展開。**🆕 Layer 3 — Claude Code から競合検知用の隠しコード（ステガノグラフィ）を削除（7/1 The Register）**: Anthropic が 2026-03 に Claude Code のシステムコンテキストへ仕込んだ不可視 Unicode マーカー＋エンコード済みドメインリスト（`ANTHROPIC_BASE_URL`・タイムゾーンを見て中国 AI ラボ・他 AI 企業・リセラー・ゲートウェイドメインと突合しモデル蒸留/不正リセラーを検知）を、**より強力な緩和策実装済みとして 7/1 リリース〔v2.1.198〕で撤去**の見込み。6/24 Alibaba 蒸留告発と同じ能力保護の文脈。**ステータス更新**: Claude apps gateway 新規 → ✅ 提供開始、Claude Science 新規 → ✅ 提供開始（隣接プロダクト）、Claude Code 競合検知隠しコード 導入(2026-03) → ✅ 撤去(7/1)。新 CLI 機能・新モデル・新ティアの発表なし。**CURRENT_FEATURES.md**: ① ヘッダー書換、② 主要機能に「Claude apps gateway（自己ホスト型コントロールプレーン）✅」セクションを新設、③ 予定・未確認情報に「Claude Science（✅ 隣接プロダクト）」「Claude Code 競合検知隠しコード削除（🔍）」を追記（[調査レポート](reports/2026-07-03_claude-apps-gateway-claude-science-and-covert-code-removal.md)）
- 2026-07-02: **フルモード調査**（7/1 レポート以降の差分）。**Layer 1 — CLI v2.1.198（7/1）**: **Claude in Chrome GA** 告知 + **`Notification` フックで BG エージェント通知**（`agent_needs_input`/`agent_completed`）+ **新スキル `/dataviz`**（カラーパレット検証器同梱）+ **BG エージェントがコード完了時に自動 commit/push/ドラフト PR 作成** + **Explore エージェントがメインセッションのモデル継承（opus 上限、従来 haiku 固定）** + **サブエージェント/コンパクションが extended thinking 設定継承** + **Gateway に Claude Platform on AWS（anthropicAws）追加** + **`/agents` ウィザード廃止**（今後は Claude に依頼 or `.claude/agents/` 直接編集）+ ネットワーク耐性（ECONNRESET リトライ）・BG タスク固着・エージェントチーム多数修正。新 CLI フラグ・新 Hook イベントなし。**🆕🚨 Layer 2/3 — Claude Fable 5 / Mythos 5 アクセス復旧（7/1）**: 6/12 輸出管理指令で停止していた両モデルを、米商務省が 6/30 夜に規制解除、**Fable 5〔公開版〕は 7/1 に Claude Platform/Claude.ai/Claude Code/Cowork でグローバル復旧**（7/7 まで週次上限の最大 50%→以降 usage credits、新安全分類器が jailbreak を 99% 超ブロック）、**Mythos 5 は 6/26 承認の米国機関限定で復旧**。**🆕 Layer 2 — Model Entitlements（Enterprise ベータ・7/1）**: 管理者がユーザーの利用可能モデル・effort レベル設定を制御。**ステータス更新**: Claude in Chrome 🧪 → ✅ GA、Claude Fable 5 ⛔ 停止 → ✅ グローバル復旧、Claude Mythos 5 🔮 限定再開フェーズ → 🔬 米国機関限定で復旧、Notification フック BG 通知 / `/dataviz` スキル / BG 自動 PR / Explore モデル継承 / extended thinking 継承 新規 → ✅ GA、Model Entitlements 新規 → 🧪 Enterprise ベータ。**本リポジトリ訂正**: 前回 7/1 レポートの『Fable 5 は 7/1 も ⛔ 停止継続』を『7/1 グローバル復旧』へ訂正。**CURRENT_FEATURES.md**: ① ヘッダー書換、② Chrome 拡張セクションを GA へ更新、③ Hooks 表に `Notification` BG 通知を追記、④ スラッシュコマンド表に `/dataviz` を追記、⑤ Fable 5/Mythos 5 セクションのステータスを復旧へ更新（[調査レポート](reports/2026-07-02_v2.1.198-chrome-ga-and-fable5-restored.md)）
- 2026-07-01: **フルモード調査**（6/30 レポート以降の差分）。**Layer 1 — CLI v2.1.197（6/30）= Claude Sonnet 5 を Claude Code のデフォルトモデルに切替**（機能追加・バグ修正エントリなし、モデル切替が単独の目玉、Layer 1 最新は v2.1.197）。**🆕🚀 Layer 2 — Claude Sonnet 5 GA（6/30）**: Sonnet 系次世代・Sonnet 4.6 ドロップイン後継、モデル ID `claude-sonnet-5`、**1M トークンコンテキストがデフォルト（かつ最大）**・最大出力 128k、**導入価格 $2/$10 per MTok（8/31 まで）→ 標準 $3/$15**（4.6 据え置き）、**Free/Pro のデフォルト**・Max/Team/Enterprise 利用可、Claude Code/API/Cursor/VS Code/GitHub Copilot ＋ Bedrock/Vertex/Foundry(preview) 提供。ベンチ = SWE-bench Pro 63.2%（4.6 58.1%／Opus 4.8 69.2%／GPT-5.5 58.6%、Sonnet 首位）・Terminal-Bench 2.1 80.4%（GPT-5.5 83.4% がわずかに上）・GDPval-AA v2 1,618（Opus 4.8 の 1,615 をわずかに超え）。**挙動変更**: adaptive thinking デフォルト ON・手動 extended thinking 廃止（400）・sampling パラメータ非デフォルト値 400・新トークナイザで同一テキスト約 30% 多いトークン（Opus 4.7 導入）・Priority Tier 非対応・Sonnet ティア初のリアルタイムサイバーセキュリティセーフガード。**重要 — Claude Code デフォルトが Opus 4.8 → Sonnet 5 に移行**（5/28 v2.1.154 以来の Opus デフォルトを覆すコスト効率重視の転換、Opus 4.8 は `/model` で選択継続）。**Layer 3 — Claude Fable 5〔公開版〕は 7/1 も ⛔ 停止継続**（6/12 米政府輸出管理指令、Mythos 5 は 6/26 部分解除フェーズ、Sonnet 5 含む通常モデルは無影響）。**ステータス更新**: Claude Sonnet 5 新規 → ✅ GA、Claude Code デフォルトモデル Opus 4.8 → Sonnet 5、v2.1.197 → ✅ GA、新スラッシュコマンド・新 Hook・新 CLI フラグなし。**CURRENT_FEATURES.md**: ① ヘッダー書換、② デフォルトモデル行を Sonnet 5 へ更新、③ Claude Sonnet 5 セクション新設、④ デフォルト努力レベル行に Sonnet 5 注記（[調査レポート](reports/2026-07-01_v2.1.197-claude-sonnet-5-default-model.md)）
- 2026-06-29: **ニュースモード調査**（6/28 レポート以降の差分）。新規 CLI リリースなし（Layer 1 最新 v2.1.195、Layer 1 スキップ）。**⚠️🆕 訂正 — 6/15 サブスク課金構造変更は『施行されず一時停止』だったことが判明**: 5/13-14 発表・6/15 発効予定だった programmatic 利用（Agent SDK / `claude -p` / Claude Code GitHub Actions / サードパーティ Agent アプリ）の別建てドル建て monthly credit メーター〔Pro 約 $20・Max 5x 約 $100・Max 20x 約 $200/月、ロールオーバーなし〕への分離計画は、**発効当日の 6/15 に Anthropic が Help Center で「もはや実施しない（no longer happening）」と公表し一時停止＝施行されなかった**。これらの利用面は引き続き通常サブスク上限から消費され変更なし（クレジット付与なし）、Anthropic は計画を練り直し中で将来変更時は事前告知すると表明（The New Stack『発効当日に一時停止』6/16・digitalapplied・TechTimes・Zed Blog 一致、support.claude.com 確認）。**本リポジトリは 6/13-14・6/16 レポートで『6/15 施行済み（📢→✅）』と記録していたが、実際には施行されず一時停止のため訂正**。**既反映の再確認（新規ではない）**: Trusted Devices for Remote Control（🧪 ベータ・6/25）/ Code with Claude London の Self-hosted sandboxes（🧪 公開ベータ・5/19）+ MCP tunnels（🔬 研究プレビュー・5/19）/ Claude Code Artifacts（🧪 ベータ・6/18）はいずれも反映済み。公式ニュースルームは 6/25 Trusted Devices 以降 6/26〜29 に新規 Claude Code 機能発表なし。**ステータス更新**: 6/15 サブスク課金構造変更 ✅ 施行済み → ❌ 一時停止（施行されず・訂正）、新 CLI 機能・新モデル・新ティアなし。**CURRENT_FEATURES.md**: ① ヘッダー書換、② 廃止予定・重要な変更スケジュール表の「サブスク課金構造変更」行を「6/15 一時停止＝施行されず」へ訂正（[調査レポート](reports/2026-06-29_june15-billing-change-paused.md)）
- 2026-06-28: **ニュースモード調査**（6/27 レポート以降の差分）。新規 CLI リリースなし（Layer 1 最新 v2.1.195、Layer 1 スキップ）。**🆕 米政府が Claude Mythos 5 の輸出規制を部分解除（6/26 金・Lutnick 商務長官書簡）** = 6/12 指令で全面停止していた Mythos 5 を 100+ の「信頼できる」米国機関（Glasswing 約 100 社＋連邦機関、クリティカルインフラ）へ展開許可、**Fable 5〔公開版〕は書簡で言及されず停止継続**（政策上の許可付与で実展開完了は別レイヤー、前回 6/27 報告のゼロトラフィックと矛盾せず）。報道ベース（CNBC/The Hill/9to5Mac/BeInCrypto/Yahoo/SRN〔Semafor〕一致）、公式ブログでの正式アナウンス・実展開タイムライン未確認。**🆕 Trusted Devices for Remote Control（Claude Code）🧪 ベータ（6/25 Help Center）** = Team/Enterprise の管理者がローカル Claude Code セッションのリモート閲覧・操作前にデバイス検証を必須化できる組織横断設定、デフォルトオフ（前回 6/27 レポートで未捕捉のため新規追加）。公式ニュースルームは 6/23 Claude Tag for Slack 以降 6/24〜28 新規投稿なし。Apple Foundation Models 連携（`ClaudeForFoundationModels`）は 6/8〜09 既出につき除外。**ステータス更新**: Mythos 5 ⛔ 全面停止 → 🔮 100+ 米国機関への限定再開フェーズ、Fable 5〔公開版〕⛔ 停止継続、Trusted Devices for Remote Control 新規 → 🧪 ベータ、新 CLI 機能・新モデル・新ティアなし。**CURRENT_FEATURES.md**: ① ヘッダー書換、② Fable 5/Mythos 5 セクションに 6/26 Mythos 5 輸出規制部分解除＋現況リリース状態を追記、③ Remote Agents セクションに Trusted Devices を追記、④ 予定・未確認情報の復旧見通しエントリを 6/26 解除で更新（[調査レポート](reports/2026-06-28_mythos5-export-clearance-and-trusted-devices.md)）
- 2026-06-27: **フルモード調査**（6/26 レポート以降の差分）。**Layer 1 — CLI が v2.1.194（changelog/Releases エントリなし＝欠番）と v2.1.195（6/26 21:29）の 2 本進行**（**v2.1.194 は欠番**＝v2.1.171・180・182・184・188・189・192 に続く 8 つ目、Layer 1 最新は v2.1.195）。**v2.1.195 はバグ修正・信頼性改善主体だが新環境変数 1 件あり** — **`CLAUDE_CODE_DISABLE_MOUSE_CLICKS`**（フルスクリーン時のマウスクリック/ドラッグ/ホバーを無効化、ホイールスクロールは維持。v2.1.187 のフルスクリーンマウスクリック対応をオプトアウトしテキスト選択/コピペ干渉を回避）。加えて改善 = **`claude agents` 完了リストが利用可能な縦スペースを充填しライブセッションを常時可視化** + **Remote セッション起動にプロビジョニングチェックリスト** + **Linux 音声モードが「マイクなし」と「SoX 未インストール」を区別**。バグ修正 = **Hook matcher のハイフン入り識別子（`code-reviewer`/`mcp__brave-search`）が部分一致 → 厳密一致化**（v2.1.191 のカンマ区切り matcher 修正の続き）・**音声ディクテーション**（macOS 長時間セッションで入力デバイス変更後に無音キャプチャ・スペースで区切らない言語〔日本語/中国語/タイ語〕で自動送信不発）・**プロジェクト `.claude/settings.json` のみで有効化された外部プラグインが明示インストール同意を要求していなかった問題**（セキュリティ）・**`/plugin` Enable/Disable がプラグイン名≠マーケットプレイスエントリ名で機能しない問題**・**バックグラウンドジョブが新しい Claude Code バージョンで書かれた際に `claude agents` から消失/データ損失**・**クラッシュした BG タスク再オープン時の約 5 秒ブランク画面**・**制御ソケット失敗時に到達不能のまま稼働する BG エージェントデーモン**。**新スラッシュコマンド・新 Hook イベントの追加なし**。**Layer 2/3 — 新 Claude Code 製品機能の公式発表なし**: Anthropic 公式ニュースルームは **6/23「Introducing Claude Tag」以降、6/24〜27 に新規投稿なし**。**🆕 Layer 3 — Anthropic が Alibaba を蒸留目的の不正アクセスで告発（6/24 Bloomberg）**: Alibaba Group が約 25,000 の不正アカウントを用い 4/22〜6/5 に 2,880 万件超のやり取りで Claude の **ソフトウェアエンジニアリング能力・エージェント推論能力**（＝Claude Code が体現する中核能力）を標的にモデル蒸留を狙ったキャンペーンを展開したと主張（報道ベース、Anthropic 公式声明の位置づけは要追跡）。Claude Code 自体の機能変更ではないが能力保護を巡る背景文脈として記録。**Claude Fable 5 / Claude Mythos 5 は 6/26 時点も停止継続**（6/12 米商務省 BIS 輸出管理指令、復旧の公式発表なし）= 6/25 のバイラルな「復旧した」主張に対し Anthropic スタッフが「Fable/Mythos のトラフィックはゼロ」と再度明言、モデルピッカーでの目撃は UI バグと説明（6/27 時点も変化なし）。Opus 4.8 含む他全モデルは無影響。**ステータス更新**: `CLAUDE_CODE_DISABLE_MOUSE_CLICKS` / Hook matcher 厳密一致化 / 音声ディクテーション修正群 / 外部プラグイン明示同意 / BG ジョブ・エージェント信頼性改善 新規 → ✅ GA、Fable 5/Mythos 5 ⛔ 停止継続（6/26 ゼロトラフィック再確認）、Alibaba 蒸留告発は Layer 3 文脈情報（報道ベース）、新モデル・新ティアなし。**CURRENT_FEATURES.md**: ① ヘッダー書換、② 主要なCLIフラグ表に `CLAUDE_CODE_DISABLE_MOUSE_CLICKS` を追記（[調査レポート](reports/2026-06-27_v2.1.195-disable-mouse-clicks-and-alibaba-distillation.md)）
- 2026-06-25: **フルモード調査**（6/24 レポート以降の差分）。**Layer 1 — CLI が v2.1.190（6/24 15:53）と v2.1.191（6/24 21:58）の 2 本進行**（**v2.1.188 / v2.1.189 はともに欠番**＝v2.1.171・v2.1.180・v2.1.182・v2.1.184 に続く 5・6 つ目、Layer 1 最新は v2.1.191）。**v2.1.190 はバグ修正・信頼性改善のみ**。**v2.1.191 が機能追加あり** — 目玉は **`/clear` 実行前の地点から会話を再開できる `/rewind` 対応**。加えて **MCP サーバー信頼性の複数改善**（capability discovery〔`tools/list`/`prompts`〕の自動リトライ・MCP OAuth の discovery/token リクエスト一時失敗時の 1 回リトライ・HTTP 404 エラーに URL 表示）+ **サンドボックスのネットワーク権限ダイアログ改善**（"Yes" 許可ホストの扱い）+ **ストリーミング応答中の CPU 使用量を約 37% 削減**（テキスト更新の coalesce）+ **長時間セッションのターミナル出力キャッシュによるメモリ増加抑制**。バグ修正（ストリーミング中のスクロール位置ジャンプ・停止した bg エージェントの復活・`/voice` の汎用 not available メッセージ・Windows Terminal の `/login` URL 折り返し途切れ・Ghostty over ssh/tmux のフルスクリーン Cmd+click・`claude agents` が組み込みスラッシュコマンドを bg 誤送信・hooks のカンマ区切り matcher サイレント無効化・`/permissions` Recently-denied の承認永続化・`forceRemoteSettingsRefresh` 等）。**新スラッシュコマンド・新 Hook イベントの追加なし**（`/rewind` は既存コマンドの機能拡張）。**Layer 2 — 🆕 Claude Tag for Slack（6/23 Anthropic 公式「Introducing Claude Tag」、6/24 レポートで未捕捉）**: Slack で `@Claude` をタグ付けしてタスクを委任できる **チーム共有型 AI ティームメイト**。**Enterprise / Team 向けベータ**、**Opus 4.8** 動作。チャンネル内で 1 つの Claude を全員が共有し、非同期で段階実行→Slack スレッドに結果投稿、チームの作業文脈を記憶、ambient モードで能動提示。**既存「Claude in Slack」アプリを置換**（管理者は 30 日以内にオプトインで移行）。Anthropic 自社では製品チームのコードの 65% を社内版 Claude Tag で生成と説明。**Layer 2/3 — Fable 5 復帰のサブスク同梱シグナル（❓ データマイン）**: ユーザーが **v2.1.190 の文字列差分**から、Fable 5 をサブスクに **恒久同梱（週次 usage 上限付き）** する準備を検出（新文字列 "You've used your Fable 5 usage for this week" 追加・旧 "purchased separately from your plan" 削除）。6/23 の usage credits 移行直後の方針転換シグナルだが、**米政府輸出管理指令によるアクセス停止（6/12〜）は依然継続**で、公式復旧アナウンスは 6/20〜25 の窓で出ていない（当該期間の Anthropic ニュースルーム新規投稿は 6/23 の Claude Tag のみ）。課金パッケージのシグナルと輸出規制の停止は別レイヤーで、本シグナルは復旧の確定材料ではない。**ステータス更新**: `/rewind` の `/clear` 前再開対応 / MCP 信頼性改善 / ストリーミング CPU 約 37% 削減 新規 → ✅ GA、**Claude Tag for Slack 新規 → 🧪 ベータ（Enterprise/Team・Opus 4.8）**、Fable 5/Mythos 5 ⛔ 停止継続（恒久同梱の文字列シグナルは ❓ データマイン）、新モデル・新ティアなし。**CURRENT_FEATURES.md**: ① ヘッダー書換、② 主要なスラッシュコマンド表 `/rewind` に `/clear` 前再開対応を追記、③ 予定・未確認情報に「Claude Tag for Slack（🧪）」「Fable 5 サブスク恒久同梱シグナル（❓）」を追記（[調査レポート](reports/2026-06-25_v2.1.190-191-rewind-before-clear-and-claude-tag-slack.md)）
- 2026-06-24: **フルモード調査**（6/23 レポート以降の差分）。**Layer 1 — CLI v2.1.187（6/23）= バグ修正主体だが 2 件のセキュリティ/ガバナンス系追加あり**（v2.1.186 に続く新規、**v2.1.184 は欠番**＝v2.1.171・v2.1.180・v2.1.182 に続く 4 つ目、Layer 1 最新は v2.1.187）: **`sandbox.credentials` 設定**（サンドボックス実行コマンドから資格情報ファイル・シークレット環境変数の読み取りをブロック）+ **組織のモデル制限を `/model` ピッカー・`--model`・`/model`・`ANTHROPIC_MODEL` に露出**（制限モデル選択時に "restricted by your organization's settings" 表示＝`availableModels`/`enforceAvailableModels` ガバナンスの UI 反映）+ **フルスクリーンのセレクトメニュー（権限プロンプト・`/model`・`/config` 等）でマウスクリック対応** + **`CLAUDE_CODE_MCP_TOOL_IDLE_TIMEOUT` 環境変数**（リモート MCP ツール呼び出しの 5 分無応答アボート、オーバーライド可）。バグ修正多数（`--resume` の "No conversation found"〔元の `-p` 実行がモデルターン未生成のケース〕・`--json-schema`/workflow `agent({schema})` の `StructuredOutput` 無限再呼び出し・Claude Code Remote セッション起動 ~2.7s 遅延・CJK ペースト文字化け・`/update` over Remote Control のハング・agents view bg ジョブ "working" 固着・チャネル接続ドロップ・エージェント停止通知の帰属/文言・サブエージェント深度トラッキング〔resume は元の spawn 深度復元・fork は深度上限カウント〕・リーク worktree 登録の自動クリーンアップ・Ghostty フルスクリーン Cmd+click URL・`claude --help` の `--bg`/`--background` 未掲載・`/share` アップロード中の Esc/Ctrl-C/Ctrl-D・VSCode 大規模セッション resume 時の応答不能 等約 20 件）。改善 = `/install-github-app` のワークフロー設定任意化・`/btw` の ←/→ 過去回答ナビ・`/plugin` の未使用プラグイン整理提示。**新スラッシュコマンド・新 Hook イベントの追加なし**。**Layer 2/3 — 新 Claude Code 製品機能の公式発表なし**: **Claude Fable 5 / Claude Mythos 5 は 6/24 時点も停止継続**（6/12 米政府輸出管理指令、復旧の公式発表なし）。6/22 でサブスク同梱無料が終了し 6/23 から usage credits 移行の建付けだがモデル停止中で実質保留。**予測市場（Polymarket）の復旧見通しが「近日」から大幅後退**＝12/31 87% / 8/31 75% / 7/31 68% / 7/1 20% / 6/26 11%（6/18-19 の Ciauri「数日以内」発言と矛盾、near-term 復旧の織り込み崩壊）。**6/23 に大規模障害**（10:02 ET 開始・12:44 ET 解決、Claude.ai/Console/API/Code/Cowork に影響、6/22 から 14 時間未満で再発、6 月の不安定さのパターンの一部）。一部メディア（Cybernews）の「障害後に顧客データ漏洩の主張を Anthropic が調査中」報道は ❓ 未確認。**ステータス更新**: `sandbox.credentials` / 組織モデル制限の UI 露出 / `CLAUDE_CODE_MCP_TOOL_IDLE_TIMEOUT` 新規 → ✅ GA、Fable 5/Mythos 5 ⛔ 停止継続（復旧見通しは予測市場で年内後ろ倒しへ後退・📢 公式幹部「数日以内」発言と乖離）、新モデル・新ティアなし。**CURRENT_FEATURES.md**: ① ヘッダー書換、② 主要な CLIフラグ表に `sandbox.credentials` / 組織モデル制限の `/model` 露出 / `CLAUDE_CODE_MCP_TOOL_IDLE_TIMEOUT` を追記（[調査レポート](reports/2026-06-24_v2.1.187-sandbox-credentials-and-org-model-restrictions.md)）
